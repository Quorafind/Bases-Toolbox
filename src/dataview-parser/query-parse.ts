...
import { QueryFields } from "./query-types";
import type { Source } from "./source-types";
import { Sources } from "./source-types";
import { Result } from "./query-types";
...
function precededByWhitespaceIfNotEof<T>(
  if_eof: (_: undefined) => T,
  parser: P.Parser<T>
): P.Parser<T> {
...
export const QUERY_LANGUAGE = P.createLanguage<QueryLanguageTypes>({
  ...
  headerClause: (q) =>
    q.queryType
      .chain((type): P.Parser<QueryHeader> => {
        switch (type) {
          case "table": {
 ...
 */
const optionalWhitespaceOrComment: P.Parser<string> = P.alt(
  P.whitespace,
  QUERY_LANGUAGE.comment
)
